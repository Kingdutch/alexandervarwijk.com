...
export default function Statistics() {
  ...
  const fetchPageviews = () => {
    ...
    fetch('https://visit.alexandervarwijk.com/pageviews', {
      ...
      .then((statistics) =>
        dispatchStatistics({ type: 'complete', statistics })
      );
  };
  ...
  const shownStatistics = useMemo(
    () =>
      statistics.filter(
        (pageview) => from < pageview.visited && pageview.visited <= to
      ),
    [from, to, statistics]
  );
  ...
  return (
    <div className="px-2">
      ...
      <button onClick={fetchPageviews} className="px-2">
        Reload pageviews
      </button>
      ...
      <p className="px-2 mt-2">
        <label>
          ...
          <input
            ...
            onChange={(e) =>
              setBetween(({ to }) => ({ from: e.target.valueAsNumber, to }))
            }
          />
        </label>
        <br />
        <label>
          ...
          <input
            ...
            onChange={(e) =>
              setBetween(({ from }) => ({ from, to: e.target.valueAsNumber }))
            }
          />
        </label>
      </p>
      {statistics.length ? (
        <p className="px-2 mt-2">
          <button
            className="mr-2"
            onClick={() =>
              setBetween({ from: statistics[0].visited, to: Date.now() })
            }
          >
            All Time
          </button>
      ...
      <p className="px-2 mt-2">
        <button className="mr-2" onClick={() => setBetween(toPreviousMonth)}>
          &lt; Previous Month
        </button>
        <button className="ml-2" onClick={() => setBetween(toNextMonth)}>
          Next Month &gt;
        </button>
      </p>
